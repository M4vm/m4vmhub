--[[
    M4vmHub
    File.:	InitScript.lua
	Desc.:	Lazy init script for dumb shit
]]
...
local MarketError = newcclosure(function()
    error("MarketplaceService functions are not supported for security reasons in M4vmHub.")
    while true do end
end)
    ...
    if self == game then
		...
		if a == "OpenVideosFolder" or a == "OpenScreenshotsFolder" then 
			return newcclosure(function()
				error("Dangerous function '" .. a .. "' is not supported for security reasons in M4vmHub.")
				while true do end
			end)
		end
...
--[[GameMeta.__tostring = newcclosure(function(self)
    --M4vmHub CoreGui Protection - we return the ClassName of certian RobloxLocked objects to prevent stupid detection methods
...
--unlockmodulescript is useless, isn't needed in M4vmHub
...
printconsole("M4vmHub successfully (re)loaded.", 126, 174, 252)